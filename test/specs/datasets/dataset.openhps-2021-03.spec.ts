...
import { GravityProcessingMethod, GravityProcessingNode, PedometerProcessingNode } from '../../../src';

describe('dataset openhps-2021-03', () => {
    let model: Model<any, any>;
    const sink: CallbackSinkNode<any> = new CallbackSinkNode(() => {}, { uid: 'sink' });
    ...
    // Pending on OpenHPS/openhps-imu#1: this dataset counts 79 steps against
    // @openhps/core 1.0.16 where it counted 81 against 0.6.4, the version the
    // committed lockfile had been pinning behind an unbounded >=0.6.4 range. Which
    // count is correct needs domain judgement, so the expectation is not re-baselined.
        ...
        for (let i = 0; i < size; i++) {
            promise = promise.then(() => {
                return new Promise((resolve, reject) => {
                    model
                        .onceCompleted(node.inputData[0].uid)
                        .then(() => {
                            resolve();
                        })
                        .catch(reject);
