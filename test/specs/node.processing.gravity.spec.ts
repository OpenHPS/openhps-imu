...
import { GravityProcessingNode, RelativeOrientationProcessingNode } from '../../src';
    ...
    describe('using orientation', () => {
        ...
        before(function (done) {
            ModelBuilder.create()
                .from(
                    new CSVDataSource(
                        'test/data/imu/2021-03-0515.10.43.csv',
                        (row: any) => {
                            ...
                            frame.evaluationFrame.addSensor(
                                new GravitySensor(
                                    'gravity',
                                    new Acceleration(
                                        parseFloat(row['gFx'].replace(',', '.')),
                            ...
                            frame.evaluationFrame.addSensor(
                                new Accelerometer(
                                    ...
                                    frame
                                        ...
                                        .value.clone()
                                        .add(frame.evaluationFrame.getSensor(GravitySensor).value),
                                    100,
                                ),
                            );
                            return frame;
        ...
        it('should filter out gravity using orientation', (done) => {
            ...
            sink.callback = (frame: EvaluationIMUFrame) => {
                const error = frame
                    .getSensor(LinearAccelerationSensor)
                    .value.distanceTo(frame.evaluationFrame.getSensor(Accelerometer).value);
                errors.push(error);
            };
            ...
            .from(
                new CallbackSourceNode(() => {
                    ...
                    frame.addSensor(
                        new Gyroscope(
                            'gyro',
                            new AngularVelocity(-0.0010652969463144809, 0, -0.0021305938926289617),
                            50,
                        ),
                    );
                    return frame;
            ...
            .from(
                new CallbackSourceNode(() => {
                    ...
                    frame.addSensor(
                        new Gyroscope(
                            'gyro',
                            new AngularVelocity(5.4010555178144175, 1.1334759508786076, -0.5773909449024486),
                            50,
                        ),
                    );
                    return frame;
